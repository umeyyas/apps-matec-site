...
(function () {
      ...
      for (var i = 0; i < plugins.waves.length; i++) {
        ...
        var waves = new SineWaves({
          ...
          resizeEvent: function () {
            ...
            gradient2.addColorStop(0.5, "rgba(219, 52, 56, 1)");
  ...
  const $output = $('#' + $form.attr('data-form-output'));
  ...
  if ($form.length) {
    $form.on('submit', function (e) {
      ...
      // Form validation
      let hasError = false;
      $form.find('[data-constraints]').each(function () {
        const $input = $(this);
        const value = $input.val().trim();
        const constraints = $input.attr('data-constraints');
      ...
      if (hasError) {
        $output.html('⚠️ Please fill in all required fields.').addClass('active');
        setTimeout(() => $output.removeClass('active'), 4000);
        return;
      }

      // Submitting...
      $output.html('Sending message...').addClass('active');
      ...
      $.ajax({
        ...
        data: $form.serialize(),
        success: function () {
          $output.html('✅ Message sent!').addClass('active');
          $form.trigger('reset');
          setTimeout(() => $output.removeClass('active'), 6000);
        },
        error: function () {
          $output.html('❌ Error sending message.').addClass('active');
          setTimeout(() => $output.removeClass('active'), 6000);
        }
      });
    });
    ...
    // Activities
    new Carousel(
      document.getElementById("activitiesGallery"),
      {
